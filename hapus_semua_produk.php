...
$tampil_detail=mysql_query("select* from tb_detail where no_transaksi='$no_transaksi01'");
while ($baris04=mysql_fetch_array($tampil_detail))
{
	$kd_produk=$baris04['kd_produk'];
	$tampil_detail00=mysql_query("select* from tb_detail where kd_produk='$kd_produk' and no_transaksi='$no_transaksi01'");
...
$hapus_semua_produk=mysql_query("delete from tb_detail where no_transaksi='$no_transaksi01'");
if ($hapus_semua_produk)
{ ?>
	<script>alert('Semua produk berhasil dihapus')</script>
	<meta http-equiv="refresh" content="0;url='index.php?p=keranjang&id=<?php echo $id;?>'">
<?php }
else
{ ?>
	<script>alert('Semua produk gagal dihapus')</script>
	<meta http-equiv="refresh" content="0;url='index.php?p=keranjang&id=<?php echo $id;?>'">
<?php } ?>
